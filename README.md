...
3. Directly upload `apb1.ino` on your Arduino. You won't need any external library.
...
- cable's pin 5 (Data pin) to D1/TX pin
- cable's pin 4 (Source pin) to +5V pin through a 220Ω resistor
- cable's pin 2 (GND pin) to GND through a 220Ω resistor

Lastly, make sure the **baud rate** of the Serial communication is set to **31250** in the `Serial.begin()` line inside of `apb1.ino`.
...
Lastly, make sure the **baud rate** of the Serial communication is set to **115200** in the `Serial.begin()` line inside of `apb1.ino`. Double-check the Hairless MIDI Serial communication baud rate settings too.
